...
class EmailValidator {
  ...
  static EmailValidationDomainType _domainType = EmailValidationDomainType.none;
  ...
  static bool _isDigit(String c) {
    final firstCodeUnit = c.codeUnitAt(0);
  ...
  static bool _isDomain(String c, bool allowInternational) {
    if (c.codeUnitAt(0) < 128) {
      if (_isLetter(c) || c == '-') {
        _domainType = EmailValidationDomainType.alphabetic;

        return true;
      }

      if (_isDigit(c)) {
        _domainType = EmailValidationDomainType.numeric;
  ...
  static bool _isDomainStart(String c, bool allowInternational) {
    if (c.codeUnitAt(0) < 128) {
      ...
      _domainType = EmailValidationDomainType.none;
  ...
  static bool _skipDomain(
    ...
    if (_index < text.length && text[_index] == '.') {
      do {
        ...
        if (_index == text.length ||
            !_skipSubDomain(text, allowInternational)) {
          return false;
        }
        // ignore: invariant_booleans
    ...
    if (_domainType == EmailValidationDomainType.numeric) {
      return false;
    }
  ...
  static bool _skipIPv4Literal(String text) {
    ...
    while (_index < text.length && groups < 4) {
      ...
      final firstCodeUnit = text[_index].codeUnitAt(0);
      while (
          _index < text.length && firstCodeUnit >= 48 && firstCodeUnit <= 57) {
        value = (value * 10) + (firstCodeUnit - 48);
        _index++;
      }
...
enum EmailValidationDomainType {
  none,
  alphabetic,
  numeric,
  alphaNumeric,
